...
void Eci::ToEci(const DateTime &dt, const CoordGeodetic &geo) {
   ...
   */
  const double theta = m_dt.ToLocalMeanSiderealTime(geo.m_longitude);

  /*
   * take into account earth flattening
   */
  const double c =
      1.0 / sqrt(1.0 + kF * (kF - 2.0) * pow(sin(geo.m_latitude), 2.0));
  const double s = pow(1.0 - kF, 2.0) * c;
  const double achcp = (kXKMPER * c + geo.m_altitude) * cos(geo.m_latitude);
  ...
  m_position.z = (kXKMPER * s + geo.m_altitude) * sin(geo.m_latitude);
...
CoordGeodetic Eci::ToGeodetic() const {
  ...
  const double r =
      sqrt((m_position.x * m_position.x) + (m_position.y * m_position.y));
